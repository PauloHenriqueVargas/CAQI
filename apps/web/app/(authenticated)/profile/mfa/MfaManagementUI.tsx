...
import type { MfaSetupResponseDto, MfaStatusDto } from '@/lib/types';

type Modo = 'idle' | 'configurando' | 'desativando';
...
export function MfaManagementUI({ initialStatus }: { initialStatus: MfaStatusDto }) {
  ...
  async function chamarBackend(acao: 'setup' | 'enable' | 'disable', body?: unknown) {
    setCarregando(true);
    setMensagem(null);
    try {
  ...
  async function confirmarAtivacao() {
    ...
    const r = (await chamarBackend('enable', { codigo })) as MfaStatusDto | null;
    if (r?.enabled) {
      setStatus(r);
      setModo('idle');
      setSetupData(null);
      setCodigo('');
      setMensagem({ tipo: 'sucesso', texto: 'MFA habilitado com sucesso. Próximos logins exigirão o código.' });
    }
  }
    ...
    return (
      <section className="rounded-lg border-2 border-success/40 bg-white p-6 shadow-sm">
        ...
        <button
          ...
          className="mt-4 rounded border border-danger/40 px-4 py-2 text-sm text-danger hover:bg-danger/5"
        >
          Desativar MFA
        </button>
      </section>
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-600">
        ...
        passa a exigir o código de 6 dígitos do app autenticador.
      </p>
      <button
        type="button"
        disabled={carregando}
